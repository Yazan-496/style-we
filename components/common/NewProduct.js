...
const NewProduct = (props) => {
    ...
    return (
        <div className={`boxShadow group relative border border-ash ${groupClass}`}>
            <div
                onClick={() => setFavorite()}
                className="cursor-pointer absolute top-2 right-2 p-2 z-10 md:z-50 lg:z-50 flex items-center justify-center bg-gray-100 rounded-full"
            >
        ...
        <Link
            onClick={isInCart && onCLose}
            href={product?.slug ? `/product/${product?.slug}` : `#`}
            className={`flex flex-col relative bg-white`}
        >
        <div
            className={`absolute z-10 left-[39px] ${100 - parseInt((product?.offer_price * 100) / product?.price) > 70 ? "block" : "hidden"}`}>
    ...
    <Link className="flex items-center justify-center block w-full" shallow href={product?.slug ? `/product/${product?.slug}` : `#`}>
            ...
            <div className=" w-full text-center">
                <div className=" flex flex-wrap component__product-price notranslate product-item with-origin">
                    <div className=" px-1 w-full flex flex-col items-center justify-center component__product-price-special">
                        <h3 className=" flex-1 text-center truncate w-full text-sm cm-goods-list-title text-gray">
                            {product?.name}
                        </h3>
                        {showRedeem ?
